     ...
     if (msg instanceof DatagramNtf && msg.protocol == PING_PROTOCOL && addr==101)
     {
        ...
        add new TickerBehavior(100,{
             ...
             double g=fun(x1,y1,z1)
                  if(g>25 && g<35)
                  {
                     ...
                     if(g<30.1 && g>29.9)
                     {
                        ...
                        {
                          first=node.location
                          println "Circle chalu no. "+(k-1)
                          c++
                        }
                        if(x1<0 && y1<0)
                        {
                          //second=node.location
                          //println "Quarter "+node.location
                          c=10
                        }
                        if(c>5 && ((y4<0 && y1>0 && x1>0) || (x1>0 && y4>0 && y1<0)))
                        {
                            flag=1
                            println "Circle complete no "+(k++ - 1)
                            c=0
                            //k=0
                         }
                     }
         ...
         if(flag==1)
         {
            ...
            {
              println "Center pahuche @ "+node.location
              flag=2
              if(k==1)
              {
                flag=3
