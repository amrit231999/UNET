...
class f extends UnetAgent {
  ...
  private int addr
  int sens=0,c=0,k=1,flag=0,i,angle=0,r=0,ms=0;
  int counter=0;
  int cn;
  double x4,y4,z4;
  ...
  void processMessage(Message msg) {
     ...
     if (msg instanceof DatagramNtf && msg.protocol == PING_PROTOCOL && addr==101 && msg.from!=102)
     {
             ...
             double g=fun(x1,y1,z1)
                  if(g>25 && g<35)
                  {
                    //println "MOVING IN A CIRCLE: Current location of ${addr} "+[x1,y1,z1];
                     ...
                     //println(counter+" " +cn+ " " + ms)
                     if(counter%140==0 && ms==1 && (counter-cn)>=30)
                     {
       ...
       if(flag==4)                                                            //towards max depth
       {
        ...
        {
          //println "READED MAX DEPTH "+node.location
          node.diveRate=0
          flag=3
        }
       }
  ...
  if (msg instanceof DatagramNtf && msg.protocol == PING_PROTOCOL && addr==101 && msg.from==102)
  {
                        ...
                        println "Sending YO from "+ addr+"to 102"
                        phy << new ClearReq()
                        phy << new DatagramReq(to:102 ,data :"YO" , protocol:PING_PROTOCOL)
                        cn=counter
                        ms=1
